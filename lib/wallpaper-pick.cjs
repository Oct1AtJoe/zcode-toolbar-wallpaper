...
function buildPickButtonExpression(btnId, action, port, currentDim) {
  ...
  return [
    ...
    // Snap to nearest multiple of 10 so it aligns with slider step.
    "  var initDim=window[" + JSON.stringify(WIN_VAR) + "]!=null?window[" + JSON.stringify(WIN_VAR) + "]:" + JSON.stringify(serverDim) + ";",
    "  initDim=String(Math.round(Number(initDim)/10)*10);",
    ...
    "  slider.type='range';slider.min='0';slider.max='100';slider.step='10';slider.value=initDim;",
