...
function createServer(opts) {
  ...
  function handle(req, res) {
    ...
    // GET /fonts/:name -- serve Assistant woff2 for @font-face (skin-inject.cjs).
    // CORS already set globally above. Keeps injected CSS tiny vs base64.
    if (req.method === "GET" && url.indexOf("/fonts/") === 0) {
      var fontName = url.slice("/fonts/".length);
      // sanitize: word chars + dash + dot, must end .woff2
      if (!/^[\w.\-]+\.woff2$/i.test(fontName)) {
        res.writeHead(400);
        res.end("bad font name");
        return;
      }
      var fontPath = path.join(__dirname, "fonts", fontName);
      ...
      req.on("end", function () {
        ...
        if (req2.action === "pickWallpaper") {
          ...
          resizeChild.on("exit", function (rc) {
            ...
            if (req2.dim != null && isFinite(Number(req2.dim))) {
              env.ZCODE_WP_DIM = String(Math.max(0, Math.min(100, Number(req2.dim))));
            }
            ...
            injectChild.on("exit", function (rc2) {
              ...
              if (rc2 === 0) {
                try {
                  var state = require("./state.cjs");
                  var prev = state.readState(wallpaperStatePath) || {};
                  var newDim = (req2.dim != null && isFinite(Number(req2.dim)))
                    ? Math.max(0, Math.min(100, Number(req2.dim)))
                    : (prev.dim != null ? prev.dim : null);
                  state.writeState(wallpaperStatePath, {
                    file: base + ".jpg",
                    setAt: Date.now(),
                    source: "pick",
                    dim: newDim
        ...
        // --- injectSkin / removeSkin (spawn skin-inject.cjs) ---
        if (req2.action === "injectSkin" || req2.action === "removeSkin") {
          if (activeJob) return sendJson(res, 409, { accepted: false, reason: "busy" });
          var sJid = "j_" + crypto.randomBytes(3).toString("hex");
          activeJob = sJid;
          jobs.set(sJid, { state: "running", startedAt: Date.now() });
          var sArgs = req2.action === "removeSkin"
            ? [path.join(root, "lib", "skin-inject.cjs"), "--remove"]
            : [path.join(root, "lib", "skin-inject.cjs")];
          var sChild = child_process.spawn(process.execPath, sArgs, { cwd: root });
          var sOut = "";
          sChild.stdout.on("data", function (c) { sOut += c; });
          sChild.stderr.on("data", function (c) { sOut += c; });
          var sTimer = setTimeout(function () { try { sChild.kill(); } catch (e) {} }, 30000);
          sChild.on("exit", function (rc) {
            clearTimeout(sTimer);
            jobs.set(sJid, { state: rc === 0 ? "done" : "failed", exitCode: rc, output: sOut.slice(-500), finishedAt: Date.now() });
            activeJob = null;
          });
          return sendJson(res, 200, { jobId: sJid, accepted: true });
        }
    ...
    function tryListen(port) {
      ...
      server.listen(port, host, function () {
        ...
        pickBtnTimer = setInterval(function () {
          var state = require("./state.cjs");
          var st = state.readState(path.join(root, ".wallpaper.json"));
          var currentDim = (st && st.dim != null) ? st.dim : 60;
          wallpaperPick.injectPickButton(boundPort, currentDim).catch(function () {});
        }, 3000);
