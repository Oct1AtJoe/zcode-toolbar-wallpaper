...
// Keep in sync with resize.cjs IMAGE_EXTS (raster only; no .gif/.svg).
// pickWallpaper 链路靠 resize.listSourceImages 选图 + 生成 .jpg 缩图，
// resize 不处理 .gif/.svg，这里若多列会导致选图后无缩图可注入。
...
async function main() {
  ...
  for (var attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    // Saves up to (MAX_ATTEMPTS-1)*ATTEMPT_DELAY_MS when inject lands first try.
